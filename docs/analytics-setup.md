...
The confirmed identifiers are GTM container `GTM-TS9MQX78` and GA4 Measurement ID `G-H5N8JSM0SS` for `https://beylikduzuyazilim.com.tr/`. Both are recorded centrally. The completed bilingual public notice is at `https://beylikduzuyazilim.com.tr/privacy.html`, so the consent interface and post-consent GTM loader are active. Recording the GA4 ID does not itself configure GTM, publish its workspace changes, or prove that GA4 receives data.

Google Ads tags and campaigns are not enabled by this repository change. The Google tag and `contact_click` tag described by the owner remain unpublished GTM workspace changes until separately published in GTM.
...
Current status: both activation values are configured. A visitor who has not granted analytics sees the choice interface and generates no Google measurement request. GTM loads only after affirmative consent.
...
| --- | --- | --- |
| `contact_click` | `contact_method`: `whatsapp`, `phone`, or `email`; `cta_location`: `navigation`, `hero`, `service_web`, `service_mobile`, `service_software`, `team`, `final`, `footer`, `privacy_footer`, `mobile_sticky`, or `floating_whatsapp`; `language`: `tr` or `en` | A click on an element with `data-contact`. |
...
- The footer preference control lets a visitor reject after previously accepting. Withdrawal first persists denial, disables further site events, sends the consent update to the already-loaded tag, removes accessible first-party `_ga`/`_ga_*` cookies, and reloads. The fresh page does not load GTM, preventing continued automatic measurement under this basic-consent model.
- Storage access is wrapped so blocked or unavailable `localStorage` does not break navigation or contact links.
- The consent and language choices are origin- and browser-profile-scoped `localStorage` records with no code-defined expiry. If storage is blocked, the choice works for the current document but may not persist after navigation or reload.

Google recommends consent defaults before measurement commands and consent updates when the visitor changes their choice. The custom basic-consent flow queues denied defaults and the granted update before initializing GTM. Any future move to GTM-managed consent should use GTM consent APIs or a suitable CMP template rather than a Custom HTML consent command.
...
6. Confirm the GA4 property event-data retention setting and update the public notice if a fixed operator-selected duration should be stated. The public site cannot expose this account setting.
7. Reassess whether a CMP is needed if the site adds consent categories, regions, vendors or advertising; the current UI covers one optional analytics category.
...
The real GTM container, GA4 Measurement ID and privacy URL are recorded. Configure and publish GA4 inside GTM using the build sheet above; do not add a second direct Google tag for the same events.

## Manual end-to-end verification

Run these checks in a clean browser profile and repeat at 360 px, 768 px, and 1440 px widths:

1. Clear site data. Confirm the preference UI appears before any request to `googletagmanager.com`, `google-analytics.com`, or Google Ads measurement endpoints.
2. Confirm WhatsApp, phone, and email links navigate when analytics is rejected and when storage access is blocked.
3. Reject consent. Confirm GTM does not load; reload and confirm it remains unloaded.
4. Accept and confirm the GTM container loads once, with denied defaults followed by the analytics-only grant before `gtm.js` initialization.
5. Confirm `ad_storage`, `ad_user_data`, and `ad_personalization` remain denied.
...
9. Reopen cookie preferences, reject, and confirm the page reloads, accessible `_ga` cookies are removed and no GTM or GA4 request occurs in the fresh document.
10. Re-accept and confirm one new GTM initialization with no duplicate loader in that document.
11. Validate consent states and tag consent requirements in GTM Preview before publishing the container, then confirm events and parameters in GA4 DebugView.

Repository deployment activates the consent-gated GTM loader, not the unpublished GTM workspace. GTM Preview, GA4 DebugView verification and an explicit container publication remain account-side steps.
